...
/// Единая модель украшения камеры (цветовой фильтр или AR-маска).
...
class DecorationCatalog {
  ...
  static List<DecorationItem> _build() {
    ...
    final maskIcons = <String, IconData>{
      'cat': PhosphorIcons.smiley(),
      'sunglasses': PhosphorIcons.sun(),
      'heart_glasses': PhosphorIcons.heart(),
      'crown': PhosphorIcons.star(),
      'dog': PhosphorIcons.smiley(),
    ...
    for (final m in MaskCatalog.all) {
      if (m.id == 'calibration') continue;
      items.add(DecorationItem(
        id: 'm_${m.id}',
        label: m.label,
        category: DecorationCategory.mask,
        previewColor: const Color(0xFF5ABFFA),
        previewIcon: maskIcons[m.id] ?? PhosphorIcons.smiley(),
        mask: m,
      ));
    }
