...
class CameraGridPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    // #8: draw a faint dark line first so the white grid stays visible on any
    // scene (bright or dark), then the white line on top.
    final shadow = Paint()
    ...
    void grid(Paint p) {
      ...
      canvas.drawLine(Offset(2 * size.width / 3, 0), Offset(2 * size.width / 3, size.height), p);
      canvas.drawLine(Offset(0, size.height / 3), Offset(size.width, size.height / 3), p);
      canvas.drawLine(Offset(0, 2 * size.height / 3), Offset(size.width, 2 * size.height / 3), p);
    }
...
class _CameraWaveformState extends State<CameraWaveform>
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (_, __) {
        final t = _ac.value;
        // #13: taller, more legible bars with a 4th for richer rhythm.
        final heights = [
          5.0 + 8.0 * math.sin(t * math.pi),
          5.0 + 8.0 * math.sin(t * math.pi + 0.9),
          5.0 + 8.0 * math.sin(t * math.pi + 1.8),
          5.0 + 8.0 * math.sin(t * math.pi + 2.7),
        ];
        return Row(
