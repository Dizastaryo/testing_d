    ...
    return EmojiPicker(
      ...
      config: Config(
        ...
        categoryViewConfig: CategoryViewConfig(
          ...
          backgroundColor: isDark ? c.surface : c.surface2,
...
class _StickerTab extends ConsumerWidget {
  ...
  Future<void> _confirmDelete(
      BuildContext ctx, WidgetRef ref, StickerModel sticker) async {
    final ok = await showSeeUConfirm(
      ctx,
      title: 'Удалить стикер?',
      message: 'Стикер будет удалён без возможности восстановления.',
    ...
    if (!ok) return;
    try {
      await ref.read(stickerListProvider.notifier).deleteSticker(sticker.id);
    } catch (_) {
      if (ctx.mounted) {
        showSeeUSnackBar(ctx, 'Не удалось удалить стикер',
            icon: PhosphorIcons.warning(), tone: SeeUTone.danger);
      }
    }
  }
}
