...
class ChatMessageBubble extends StatelessWidget {
              ...
              child: Row(
                ...
                children: [
                  ...
                  if (mine) ...[
                    ...
                    _ReadReceiptIcon(
                        isRead: msg.isRead, isDelivered: msg.isDelivered, c: c),
                  ],
                ],
              ),
            ),
  ...
  Widget _buildReplyQuoted(ReplyPreview reply, bool isMine, SeeUThemeColors c,
    ...
    const stripeColor = SeeUColors.accent;
    const titleColor = SeeUColors.accent;
    final textColor = c.ink2;
    return GestureDetector(
  ...
  Color _senderColor(String name) {
    final palettes = SeeUColors.avatarPalettes;
    final idx = (name.codeUnitAt(0) + name.length) % palettes.length;
    return palettes[idx].first;
  }
      ...
      child: Column(
        ...
        children: [
          InkWell(
            ...
            child: Container(
              decoration: BoxDecoration(
                color: isMine ? c.surface : c.surface2,
                borderRadius: BorderRadius.circular(SeeURadii.small),
              ),
              clipBehavior: Clip.antiAlias,
              child: Column(
...
class _ChatTtlCountdownState extends ConsumerState<ChatTtlCountdown> {
  ...
  void _scheduleTick() {
    ...
    if (remaining.isNegative) {
      if (!_removed) {
        _removed = true;
        // Defer to after the current frame: _scheduleTick runs synchronously
        // from initState for an already-expired message, and mutating another
        // provider during build throws "modified a provider while building".
...
class _ChatFileBubble extends StatelessWidget {
  ...
  Future<void> _open(BuildContext context) async {
    ...
    if (await canLaunchUrl(uri)) {
      ...
      showSeeUSnackBar(context, 'Не удалось открыть файл',
...
/// Единый read-receipt на пузыре: галочка(и) с accent-как-акцентом.
/// Прочитано → accent (двойная); доставлено → приглушённая двойная; отправлено
/// → одиночная. Раньше рисовалось белым по оранжу — теперь по accentSoft.
