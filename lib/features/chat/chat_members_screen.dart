...
class _ChatMembersScreenState extends ConsumerState<ChatMembersScreen> {
  ...
  Future<void> _removeMember(_Participant p, {bool selfLeave = false}) async {
    ...
    try {
      ...
      if (selfLeave) {
        ...
        if (!mounted) return;
        showSeeUSnackBar(context, 'Вы вышли из группы');
        context.go('/chat');
      } else {
        ...
        showSeeUSnackBar(context, '@${p.user.username} удалён из группы');
      }
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _addMember() async {
    ...
    try {
      ...
      if (mounted) {
        showSeeUSnackBar(context, '@${picked.username} добавлен',
  ...
  Future<void> _confirmLeave() async {
    ...
    try {
      ...
      ref.read(chatListProvider.notifier).load(silent: true);
      if (!mounted) return;
      showSeeUSnackBar(context, successMsg);
      context.go('/chat');
    } catch (e) {
      if (!mounted) return;
      showSeeUSnackBar(context, friendlyError(e), tone: SeeUTone.danger);
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          '${_members.length} ${_plural(_members.length, 'человек', 'человека', 'человек')}',
                      ...
                      GestureDetector(
                        ...
                        child: Padding(
                          ...
                          child: Row(children: [
                            Container(
                              width: 44, height: 44,
                              decoration: BoxDecoration(shape: BoxShape.circle, color: SeeUColors.accentSoft),
  ...
  Future<void> _confirmKick(_Participant p) async {
    final ok = await showSeeUConfirm(
      context,
      title: 'Удалить @${p.user.username}?',
      message: 'Этот юзер потеряет доступ к группе.',
      confirmLabel: 'Удалить',
      destructive: true,
      icon: PhosphorIcons.userMinus(),
    );
    if (ok) _removeMember(p);
  }
            ...
            child: Column(
              ...
              children: [
                ...
                ListTile(
                  ...
                  onTap: () {
                    Navigator.pop(sheetCtx);
                    showSeeUSnackBar(context, 'Скоро');
                  },
                ),
                Divider(height: 1, color: c.line),
                // Remove
