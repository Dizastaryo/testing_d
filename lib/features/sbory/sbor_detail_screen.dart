...
class _SborDetailScreenState extends ConsumerState<SborDetailScreen> {
              ...
              children: [
                ...
                Text(
                  ...
                  style: SeeUTypography.displayM.copyWith(
                    ...
                    height: 1.15, color: Colors.white,
                  ),
                ),
                const SizedBox(height: 8),
                Row(
  ...
  Widget _buildStatusBanner(SeeUThemeColors c, Sbor s) {
    ...
    if (isOrganizer) {
      ...
      bg = SeeUColors.amber.withValues(alpha: 0.15);
      fg = SeeUColors.warning;
              ...
              child: FutureBuilder<List<dynamic>>(
                ...
                builder: (_, snap) {
                  ...
                  if (snap.hasError || !snap.hasData) {
                    ...
                    final isForbidden = err is DioException &&
                        err.response?.statusCode == 403;
                    return Padding(
                      padding: const EdgeInsets.all(24),
                      child: Text(
                        isForbidden
                            ? 'Вступите в сбор, чтобы видеть всех участников'
                            : 'Ошибка загрузки',
                      ...
                      Expanded(
                        child: GestureDetector(
                          ...
                          child: Container(
                            height: 46,
                            decoration: BoxDecoration(
                              // Ghost-кнопка; при ожидающих заявках — accent-tint.
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                Icon(PhosphorIcons.hourglassMedium(), size: 18, color: SeeUColors.warning),
                const SizedBox(width: 8),
                const Text(
  ...
  Future<void> _toggleBookmark(Sbor s) async {
    ...
    try {
      ...
      showSeeUSnackBar(
        context,
        saved ? 'Добавлено в сохранённые' : 'Убрано из сохранённых',
  ...
  Future<void> _sendToChatDirect(Sbor s) async {
    ...
    if (chats.isEmpty) {
      showSeeUSnackBar(context, 'Нет доступных чатов');
      return;
    }
  ...
  Future<void> _sendSborToChat(String chatId, Sbor s) async {
    try {
      ...
      if (!mounted) return;
      showSeeUSnackBar(context, 'Сбор отправлен в чат', tone: SeeUTone.success);
    } catch (e) {
      if (!mounted) return;
      showSeeUSnackBar(context, 'Ошибка: $e', tone: SeeUTone.danger);
    }
  ...
  Future<void> _submitRequest(Sbor s) async {
    ...
    try {
      ...
      if (!mounted) return;
      ref.invalidate(_sborDetailProvider(widget.sborId));
      showSeeUSnackBar(context, 'Заявка отправлена — ждём подтверждения',
  ...
  Future<void> _leaveSbor(Sbor s) async {
    final confirmed = await showSeeUConfirm(
      ...
      message: 'Ты покинешь сбор и групповой чат.',
      confirmLabel: 'Выйти',
      cancelLabel: 'Нет',
      destructive: true,
  ...
  Future<void> _cancelSbor(Sbor s) async {
    final confirmed = await showSeeUConfirm(
      context,
      title: 'Отменить сбор?',
      message: 'Все участники будут уведомлены об отмене.',
