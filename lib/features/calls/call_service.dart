...
import 'group_call_service.dart' show GroupCallService, GroupCallStatus;
...
class CallService {
  ...
  Future<void> startCall({
    ...
    // #2: getUserMedia could have been denied (→ _cleanup set the session to
    // ended) or the user hung up while the permission dialog was open. Bail
    // before announcing the invite, else the callee gets a ghost incoming call
    // for a session that no longer exists on our side. Mirrors acceptIncoming.
  ...
  Future<void> _handleIncomingInvite(
    ...
    // Decline if a group call is active — accepting would break event routing.
    final gs = GroupCallService.instance.session.value;
    if (gs != null && gs.status != GroupCallStatus.idle) {
      appLog('[CallService] busy (group call active) — declining incoming from $from');
      _send('call.decline', {'to_user_id': from});
      return;
    }
