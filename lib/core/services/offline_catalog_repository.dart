...
class OfflineCatalogRepository {
  ...
  Future<void> updateProgress(
    ...
    await _ensureInit();
    return _store.updateProgress(fileId, percent, jsonEncode(position));
  }
      ...
      sub = _queue.watch(fileId).listen((p) {
        if (p.status == DownloadTaskStatus.completed) {
          ...
          _storage.localPath(fileId, kind).then((path) {
            ...
            // localPath can come back null (e.g. the blob was evicted right
            // after the download finished). Completing a Completer<String>
            // with null throws a TypeError and the future would never resolve —
            // the reader would hang forever. Surface a real error instead.
            if (path != null) {
              completer.complete(path);
            } else {
              completer.completeError(
                  Exception('File not found after download'));
            }
          }).catchError((Object e) {
