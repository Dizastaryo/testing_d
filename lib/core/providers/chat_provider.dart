...
/// Tolerant boolean parse. The backend may send `true`, `1`, or `"true"` for
/// the same flag depending on the column type / serializer. A hard
/// `as bool` cast throws on a numeric/string truthy value and (with the
/// per-entry parse guard) silently drops an otherwise valid message.
...
class ChatMessage {
  ...
  factory ChatMessage.fromJson(Map<String, dynamic> json) {
    ...
    return ChatMessage(
      ...
      isDelivered: _asBool(json['is_delivered']),
      ...
      isDeletedForAll: _asBool(json['is_deleted_for_all']),
...
class ChatMessagesNotifier extends StateNotifier<ChatMessagesState> {
        ...
        next.whenData((evt) {
          ...
          if (evt.type == 'chat.message') {
            try {
              ...
              // Insert by createdAt instead of blind append: a delayed/out-of-
              // order WS frame must not land after a newer message.
              state = state.copyWith(
                messages: _insertByCreatedAt(state.messages, msg),
              );
            } catch (e, st) {
  ...
  Future<void> load() async {
    ...
    try {
      ...
      final data = response.data['data'];
      if (data is List) {
        // Guard each entry: one malformed message must not blank the whole chat.
        final all = <ChatMessage>[];
        for (final e in data) {
          try {
            all.add(ChatMessage.fromJson(e as Map<String, dynamic>));
          } catch (parseErr) {
            appLog.warn('[ChatMessagesNotifier] skipping malformed message: $parseErr');
          }
        }
  ...
  /// Inserts [msg] into [existing] (assumed ascending by createdAt) at the
  /// correct position, after any messages with an equal timestamp so existing
  /// order is preserved for ties.
  static List<ChatMessage> _insertByCreatedAt(
      List<ChatMessage> existing, ChatMessage msg) {
    final list = [...existing];
    var idx = list.length;
    while (idx > 0 && list[idx - 1].createdAt.isAfter(msg.createdAt)) {
      idx--;
    }
    list.insert(idx, msg);
    return list;
  ...
  Future<void> loadOlderMessages() async {
    ...
    try {
      ...
      final data = response.data['data'];
      if (data is List) {
        // Guard each entry: one malformed message must not blank the page.
        final all = <ChatMessage>[];
        for (final e in data) {
          try {
            all.add(ChatMessage.fromJson(e as Map<String, dynamic>));
        ...
        // Dedup older page against what we already hold — an overlapping
        // cursor page (or a message that arrived via WS meanwhile) must not
        // be prepended twice. Then keep the merged list ordered by createdAt.
        final existingIds = state.messages.map((m) => m.id).toSet();
        final dedupedOlder =
            older.where((m) => !existingIds.contains(m.id)).toList();
        final merged = [...dedupedOlder, ...state.messages];
...
final typingProvider =
    StateNotifierProvider.autoDispose.family<_TypingNotifier, TypingState, String>(
