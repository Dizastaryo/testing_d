...
class FeedNotifier extends StateNotifier<FeedState> {
  ...
  Future<void> toggleLike(String postId) async {
    ...
    try {
      ...
      // Server rejected the like/unlike — revert the optimistic update so the
      // UI doesn't silently keep the wrong state.
  ...
  Future<void> toggleSave(String postId) async {
    ...
    if (idx < 0) return;
    // Capture the pre-update post so we can restore it if the server fails.
    ...
    try {
      ...
      // Revert the optimistic save toggle on failure.
      final i = state.posts.indexWhere((p) => p.id == postId);
      if (i >= 0) {
        state = state.copyWith(
          posts: [
